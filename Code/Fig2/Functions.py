...
def ExtractSetRFDifferentSets(instance,stepName,JobName,nodeSet1,nodeSet2):
	text_file = open(JobName+'_Output.txt', 'w')
	odb=openOdb(path=JobName+'.odb')
	set1 = odb.rootAssembly.nodeSets[nodeSet1]
	set2 = odb.rootAssembly.nodeSets[nodeSet2]
	for frame in odb.steps[stepName].frames:
		for disp,rf in  zip(frame.fieldOutputs['U'].getSubset(region = set1).values,frame.fieldOutputs['RF'].getSubset(region = set2).values):
