...
AppYLoad = -1. # applied y displacment boundary condition
THICKNESS=100.
...
JobName=JobName+'_N'+str(N_DOMAINS)
...
# CREATING SETS FOR SECTION ASSIGNMENTS
...
if N_DOMAINS>1:
    mdb.models['Model-1'].rootAssembly.LinearInstancePattern(direction1=(1.0, 0.0, 
        0.0), direction2=(0.0, 1.0, 0.0), instanceList=('Part-1-1', ), number1=N_DOMAINS, 
        number2=N_DOMAINS, spacing1=2*spacing, spacing2=2*spacing)
    mdb.models['Model-1'].rootAssembly.InstanceFromBooleanMerge(domain=GEOMETRY, 
        instances=mdb.models['Model-1'].rootAssembly.instances.values(), name=
        'Part-2', originalInstances=DELETE)

    del mdb.models['Model-1'].parts['Part-1']
    mdb.models['Model-1'].parts.changeKey(fromName='Part-2', toName='Part-1')
    mdb.models['Model-1'].rootAssembly.regenerate()
    mdb.models['Model-1'].rootAssembly.features.changeKey(fromName='Part-2-1', 
        toName='Part-1-1')

# MESHING
mdb.models['Model-1'].parts['Part-1'].seedPart(deviationFactor=0.12,
                                            minSizeFactor=0.12, size=0.12)
...
mdb.models['Model-1'].rootAssembly.Set(name='AllEdgeNode', nodes=[
    Instance_Full.nodes.getByBoundingBox(
        0 - TOL, 0 - TOL, -TOL, 2 * spacing *N_DOMAINS - TOL, 0 + TOL, TOL),
    Instance_Full.nodes.getByBoundingBox(
        0 - TOL, 2 * spacing*N_DOMAINS - TOL, -TOL, 2 * spacing * N_DOMAINS+ TOL, 2 * spacing*N_DOMAINS + TOL, TOL),
    Instance_Full.nodes.getByBoundingBox(
        0 - TOL, 0 - TOL, -TOL, 0 + TOL, 2 * spacing*N_DOMAINS + TOL, TOL),
    Instance_Full.nodes.getByBoundingBox(
        2 * spacing*N_DOMAINS - TOL, 0 - TOL, -TOL, 2 * spacing*N_DOMAINS + TOL, 2 * spacing*N_DOMAINS + TOL, TOL)])

(NameRef1, NameRef2, repConst) = PeriodicBound2D(mdb, 'Model-1',
                                                'AllEdgeNode', [(2.0 * spacing*N_DOMAINS, 0.0), (0.0, 2.0 * spacing*N_DOMAINS)])
...
for THETA in THETAALL:
    mdb.models['Model-1'].ConcentratedForce(cf1=0.0, cf2=AppYLoad, createStepName=
            'Step-1', distributionType=UNIFORM, field='', localCsys=None,  name='BC-REF-2', region=Region(referencePoints=(
    ...
    RunName=JobName+'_THETA'+str(int(THETA))

    DeleteAbaqusFiles(RunName)
    
    mdb.models['Model-1'].rootAssembly.regenerate()
    mdb.Job(atTime=None, contactPrint=OFF, description='', echoPrint=OFF,
            explicitPrecision=SINGLE, getMemoryFromAnalysis=True, historyPrint=OFF,
            memory=90, memoryUnits=PERCENTAGE, model='Model-1', modelPrint=OFF,
            multiprocessingMode=DEFAULT, name=RunName, nodalOutputPrecision=SINGLE,
            numCpus=1, numGPUs=0, queue=None, scratch='', type=ANALYSIS,
            userSubroutine='', waitHours=0, waitMinutes=0)
    mdb.jobs[RunName].submit(consistencyChecking=OFF)
    mdb.jobs[RunName].waitForCompletion()
    
    EigenValues = np.array(ExtractEigenMode(RunName, 8))*(-1.*AppYLoad) 
    CriticalStrains = [N_DOMAINS, THETA] + list(EigenValues)

    file.write(CreateEString(10) % tuple(CriticalStrains))

    mdb.models['Model-1'].rootAssembly.rotate(angle=-THETA, axisDirection=(0.0, 0.0,
                                                                        1.0), axisPoint=(spacing, spacing, 0.0), instanceList=('Part-1-1', ))

    DeleteAbaqusFilesButODB(RunName)
